...
export default function HomeClient({
  ...
  // Background refresh — keeps a long-open tab current and self-heals if the
  // server render came back empty (e.g. a transient DB hiccup).
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const { fetchHouseProjects } = await import('@/utils/productCache');
        const data = await fetchHouseProjects({ limit: 100, force: true });
        if (cancelled) return;
        const next = toVisiblePlanCards(data);
        if (next.length) {
          setProjects(next);
          const img = next.find((p) => p.image);
