...
export default function CatalogClient({
  ...
  initialProjects: PlanCardData[];
}) {
  const [projects, setProjects] = useState<PlanCardData[]>(initialProjects);
  const [filteredProjects, setFilteredProjects] =
    useState<PlanCardData[]>(initialProjects);
  // Only show skeletons when we have nothing server-rendered to show.
  ...
  // Background refresh so newly published plans / admin edits show up without
  // waiting for the page's ISR window. Silent unless we started with nothing.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const { fetchHouseProjects } = await import('@/utils/productCache');
        const data = await fetchHouseProjects({ limit: 200, force: true });
        if (cancelled) return;
        const next = toVisiblePlanCards(data);
        if (next.length) setProjects(next);
        setLoadError(false);
      } catch (error) {
        console.error('Error refreshing catalog:', error);
        if (!cancelled && projects.length === 0) setLoadError(true);
      } finally {
